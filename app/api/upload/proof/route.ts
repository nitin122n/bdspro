...
export async function POST(request: NextRequest) {
  try {
    ...
    const [transactions] = await db.execute<RowDataPacket[]>(
      ...
      [transactionId, authResult.user_id]
    );
    ...
    const fileName = `proof_${transactionId}_${timestamp}_${randomString}.${fileExtension}`;
    ...
    await db.execute(
      'UPDATE transactions SET proof_image = ? WHERE id = ?',
      [relativePath, transactionId]
    );
